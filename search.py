...
def search_helper(problem, state, actions):
    """
    ...
    """
    state.current_row_constraint += 1
    problem.board.move_to_the_next_cell()
    get_successors = True
    for move in actions:
        # Try to fill the move on the board, and check if it legal.
        state.fill(move.row, move.col, move.color)
        if not state.check_move(move.col, move.row):
            get_successors = False
            problem.board.back_to_the_prev_cell()
            while state.current_cell.color == WHITE:
                state.fill(state.current_cell.row, state.current_cell.col, EMPTY)
                problem.board.back_to_the_prev_cell()
            break
...
def depth_first_search(problem):
    ...
    while not fringe.isEmpty():
        ...
        if search_helper(problem, current[0], current[1]):
...
def get_random_stats(board, n):
    ...
    cur_k = 0
    while cur_k < n:
        r = random.randint(0, board.num_rows)
        c = random.randint(0, board.num_cols)
