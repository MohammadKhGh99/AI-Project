...
class NonogramProblem(SearchProblem):
    """
        Class that defining the nonogram game as a problem, by solving row constraints one by one.
    """
        ...
        for start_index in range(state.num_cols):
            ...
            if child is not None:
                ...
                for i in range(constraint.length):
                    actions.add((constraint_coord[0], start_index + i))
...
class NonogramCells(SearchProblem):
    """
        Class that defining the nonogram game as a problem, by checking all possible colors for each cell.
    """

    def __init__(self, board):
        self.board = board
        self.current_cell = board.get_cell(0, 0)
