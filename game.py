...
class Game:
    def __init__(self, csv_file=None, rows_constraints=None, cols_constraints=None, colors=BLACK_WHITE,
                 size=(5, 5), my_agent=None, always_solvable=True, rows_or_cols=ROWS, gui_or_print=IS_GUI, difficulty=EASY):
        """
    ...
    def __build_easy(self, m, n, temp_board, temp_constraints, rows_cols=ROWS):
        for i in range(m):
            con_lst = []
            j = 0
            while j < n:
                ...
                if color == BLACK:
                    ...
                    con = 'r'
                con_lst.append(Constraint(str(ran_len) + con))
        ...
        if not self.always_solvable:
            ...
            if self.difficulty == EASY:
                ...
                if self.rows_or_cols == COLUMNS:
                    temp_cols_constraints = self.__build_easy(self.num_cols, self.num_rows, temp_board, temp_cols_constraints, rows_cols=COLUMNS)
                    temp_rows_constraints = self.__build_rows_constraints(temp_board, temp_rows_constraints)
                else:
                    temp_rows_constraints = self.__build_easy(self.num_rows, self.num_cols, temp_board, temp_rows_constraints, rows_cols=ROWS)
